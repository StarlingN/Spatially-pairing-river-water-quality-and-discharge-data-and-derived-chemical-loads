# -*- coding: utf-8 -*-
"""
Spatially join DataStream observations (points) to CLRH catchment polygons (polys),
appending catchment attributes to each observation row, with shapefile-derived
columns written *prefixed* as "datastream_catchment_<name>" in a single pass.

- Streams the CSV in chunks to keep memory reasonable.
- Preserves ALL original CSV fields exactly as text (no type coercion).
- Polygon attributes are appended with 'datastream_catchment_' prefix; blanks if no containing polygon.
...
SHAPEFILE_PATH = r"E:\publications\clawave_1\gis\finalcat_info_v1_0_clipped.gdb"  # path to GDB
# The feature class name inside the GDB:
CATCHMENT_LAYER_NAME = "finalcat_info_v1_0_clipped"

INPUT_CSV_PATH = r"E:\publications\clawave_1\datastream\process_4_merged_unified_crs+renamed\observations_2000_2025_all_clipped_unified_crs_renamed.csv"

# OUTPUT (NOTE: writes directly to +catchments.csv as requested)
OUTPUT_CSV_PATH = r"E:\publications\clawave_1\datastream\process_5_catchments_added\observations_2000_2025_all_clipped_unified_crs_renamed+catchments.csv"
...
# Coordinate columns in the input CSV
CSV_LAT_COL = "datastream_wgs84_lat"
CSV_LON_COL = "datastream_wgs84_long"

# Original CSV columns in desired order (unchanged from your script)
CSV_COLUMNS_ORDER = [
...
# Shapefile fields (as they exist in the shapefile)
...
CATCHMENT_PREFIX = "datastream_catchment_"
...
logger = logging.getLogger("join_observations_to_catchments_prefixed")
...
def load_catchments(dataset_path: str, layer_name: str | None = None) -> gpd.GeoDataFrame:
    ...
    and RENAME those columns to have 'datastream_catchment_' prefix for output.
    """
    logger.info(f"Loading catchments from: {dataset_path}" + (f" [layer={layer_name}]" if layer_name else ""))
    ...
    # If the shapefile already has some 'datastream_catchment_' fields, carry them through as-is.
...
def main():
    ...
    with tqdm(desc="Joining points to catchments", unit="rows", unit_scale=True) as pbar:
        for i, df_chunk in enumerate(chunk_iter, start=1):
            ...
            total_valid_points += int(in_bbox_mask.sum())
            ...
            if i % SHOW_SAMPLE_LOG_EVERY_N_CHUNKS == 0:
                logger.info(
                    f"Chunk {i:>4}: rows={this_rows:,} | valid_pts_in_bbox={int(in_bbox_mask.sum()):,} | "
