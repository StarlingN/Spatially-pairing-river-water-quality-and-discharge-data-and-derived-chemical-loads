# -*- coding: utf-8 -*-
"""
Join DataStream observation points (CSV) to nearest stream polylines (shapefile),
append stream attributes (prefixed as 'datastream_stream_...') and compute geodesic distance (meters)
from each observation to the nearest stream.

- Streams the 3GB CSV in chunks.
- Preserves ALL original CSV fields exactly as text.
- Appends datastream_stream_* columns + final 'datastream_stream_distance' column.
...
INPUT_CSV_PATH = r"E:\publications\clawave_1\datastream\process_5_catchments_added\observations_2000_2025_all_clipped_unified_crs_renamed+catchments.csv"

# OUTPUT
OUTPUT_CSV_PATH = r"E:\publications\clawave_1\datastream\process_6_streams_added\observations_2000_2025_all_clipped_unified_crs_renamed+catchments+streams.csv"
...
CSV_LAT_COL = "datastream_wgs84_lat"
CSV_LON_COL = "datastream_wgs84_long"
...
CSV_COLUMNS_ORDER: List[str] = [
    ...
    "datastream_catchment_SubId","datastream_catchment_DowSubId","datastream_catchment_RivSlope","datastream_catchment_RivLength","datastream_catchment_BasSlope","datastream_catchment_BasAspect","datastream_catchment_BasArea",
    "datastream_catchment_BkfWidth","datastream_catchment_BkfDepth","datastream_catchment_Lake_Cat","datastream_catchment_HyLakeId","datastream_catchment_LakeVol","datastream_catchment_LakeDepth","datastream_catchment_LakeArea",
    "datastream_catchment_Laketype","datastream_catchment_Has_POI","datastream_catchment_MeanElev","datastream_catchment_FloodP_n","datastream_catchment_Q_Mean","datastream_catchment_Ch_n","datastream_catchment_DrainArea",
    "datastream_catchment_Strahler","datastream_catchment_Seg_ID","datastream_catchment_Seg_order","datastream_catchment_Max_DEM","datastream_catchment_Min_DEM","datastream_catchment_Obs_NM","datastream_catchment_SRC_obs",
    "datastream_catchment_centroid_x","datastream_catchment_centroid_y","datastream_catchment_DA_Chn_L","datastream_catchment_DA_Slope","datastream_catchment_DA_Chn_Slp","datastream_catchment_outletLat","datastream_catchment_outletLng",
    "datastream_catchment_k","datastream_catchment_c","datastream_catchment_SSDA_NM","datastream_catchment_SDA_NM","datastream_catchment_Shape_Leng","datastream_catchment_Shape_Area"
]
...
STREAM_PREFIX = "datastream_stream_"
STREAM_OUTPUT_COLUMNS = [f"{STREAM_PREFIX}{c}" for c in STREAM_INPUT_COLUMNS]
STREAM_DISTANCE_COL = "datastream_stream_distance"
...
logger = logging.getLogger("join_points_to_nearest_streams")
    ...
    with tqdm(desc="Nearest streams", unit="rows", unit_scale=True) as pbar:
        for i, df_chunk in enumerate(chunk_iter, start=1):
            ...
            if len(idx_valid) > 0:
                ...
                # NEW/CHANGED: collapse any tie-duplicates so there's max 1 match per point
                if joined.index.duplicated().any():
                    joined = joined[~joined.index.duplicated(keep="first")]

                # NEW/CHANGED: ensure 1:1 alignment and row order equals idx_valid
                ...
                if not joined.empty:
                    ...
                    # NEW/CHANGED: assignment is now safe because
                    # attrs_slice has shape == (len(idx_valid), len(stream_cols_all))
